...
| Screen | What it does |
| --- | --- |
| **Today** | **The Unstuck Button** (one press → one small thing, picked from your real items), Daily Brief (calendar-aware), Top 3 outcomes, 5-minute starts, open loops + task management (in-progress/waiting/drop), drafts awaiting approval with resolved history, today's Google Calendar + Block time, Evening Shutdown CTA with a calm momentum line |
...
- **Audit log**: consequential mutations (convert, delete, approve/reject, loop runs, seeding, AI writes) append `auditLog` entries, viewable in Settings → Data.
- **Dark mode** is built-in (toggle in the top bar): an inline script applies the stored choice before first paint (no flash), defaulting to the system preference — no theme library.
- **Guided Evening Shutdown** bookends the day: a three-step ritual (empty your head → mark what actually happened → choose tomorrow's first action) that records a loop run and ends with “This counted.” The chosen first action is stored with a date, greets you on Today the next morning (“Chosen last night — start here”, with judgement-free *Did it* / *Not today*), and the Daily Brief is contractually required to make it outcome #1 rather than overriding it.
- **The Unstuck Button** is the panic button for frozen moments: one press asks the AI to choose exactly one action from your real items (never a list, never invented), with an honest reason, the first five minutes spelled out, and one calm sentence. Up to three re-rolls, then it gently stops offering options. Every press is recorded in a `nudges` table — “I did it” marks it acted — and the weekly review sees those moments as information about hard days, never as failure.
...
- Loops are **time-of-day aware**: each has a window (morning / afternoon / evening / anytime) that gates when "due now" starts — Evening Shutdown won't nag at breakfast; it shows softly under “Later today” until its window opens. Window boundaries are **per-user adjustable** in Settings → Profile → Day rhythm (defaults 5am / noon / 5pm; night owls can start the evening at 8pm), along with the local hour the scheduled Daily Brief generates and emails (default 5am). Windows never expire within the day (running late is always allowed), and completed runs schedule their next due time at **local midnight** boundaries rather than a drifting 24-hours-later. Monthly cadence approximates to 30 days.
